...
// Parse args for --email
const args = process.argv.slice(2);
const emailArg = args.find(a => a.startsWith("--email="))?.split("=")[1];

const USER_EMAIL = emailArg || "demo@vindycaition.com";

async function seed() {
    console.log(`🌱 Starting seed for logic user: ${USER_EMAIL}`);

    // 1. Get User ID
    let userId: string;

    // Attempt to find user
    // We use listUsers() to bypass RLS limits on auth.users if possible
    ...
    const existingUser = usersData.users.find(u => u.email === USER_EMAIL);

    if (existingUser) {
        console.log(`✅ User found: ${USER_EMAIL} (${existingUser.id})`);
        userId = existingUser.id;
    } else {
        console.error(`\n❌ USER NOT FOUND: ${USER_EMAIL}`);
        console.error("Automatic creation is blocked by database triggers.");
        console.error("👉 Please REGISTER this user manually in the application (Sign Up page).");
    ...
    const invoicesData = [
        // --- OVERDUE --- 
        {
            ...
            actions: [
                ...
                { type: "call", status: "completed", daysAgo: 5 },
            ],
        },
        {
            ...
            actions: [
                ...
                { type: "sms", status: "failed", daysAgo: 8 },
            ],
        },
        {
            debtor: "Budex Sp. z o.o.",
    ...
    // 6. Create Cost Invoices
    console.log("Creating cost invoices...");
